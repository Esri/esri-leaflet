...
describe("L.esri.Util", () => {
  ...
  const sampleBounds = new L.LatLngBounds([
    [45.5, -122.7], // sw lat, lng
    [45.52, -122.64], // ne lat lng
  ]);

  const hostedFeatureServiceUrl =
    "http://services.arcgis.com/rOo.../arcgis/rest/services/RawsomeServiceName/FeatureServer/0";
  const otherServiceUrl =
    "http://demographics4.arcgis.com/arcgis/rest/services/USA_Demographics_and_Boundaries_2014/MapServer/9";
  const normalFeatureServiceUrl =
    "http://oneofoursampleservers.arcgisonline.com/arcgis/rest/services/WorldTimeZones/MapServer/2";

  const knownIdFieldTestCases = [
    "OBJECTID",
    "FID",
    "OID",
    "ID",
  ...
  it("shouldnt trim spaces in the middle", () => {
    const url = L.esri.Util.cleanUrl(
      "   http://arcgis.com/cool folder/anotherfolder ",
    );
  ...
  describe("_findIdAttributeFromFeature", () => {
    for (let caseId = 0; caseId < knownIdFieldTestCases.length; caseId++) {
      ...
      it(`should return the correct key when a item has ${testCase} attribute`, () => {
        ...
        feature.attributes[testCase] =
          "set up our id field amongst others here";

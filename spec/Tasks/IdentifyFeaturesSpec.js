...
describe('L.esri.IdentifyFeatures', () => {
	...
	it('should identify features with a layer definition', (done) => {
		const request = task.layerDef(0, 'NAME=Oregon').run((error, featureCollection, raw) => {
			expect(featureCollection).to.deep.equal(sampleFeatureCollection);
			expect(raw).to.deep.equal(sampleResponse);
			done();
		});
	...
	it('should identify features with 2 layer definitions', (done) => {
		const request = task.layerDef(0, 'NAME=Oregon').layerDef(1, 'NAME=Multnomah').run((error, featureCollection, raw) => {
			expect(featureCollection).to.deep.equal(sampleFeatureCollection);
			expect(raw).to.deep.equal(sampleResponse);
			done();
		});
	...
	it('should identify features in a given time range', (done) => {
		...
		const request = task.between(start, end).run((error, featureCollection, raw) => {
			expect(featureCollection).to.deep.equal(sampleFeatureCollection);
			expect(raw).to.deep.equal(sampleResponse);
			done();
		});
	...
	it('should identify features and simplify geometries', (done) => {
		const request = task.simplify(map, 0.5).run((error, featureCollection, raw) => {
			expect(featureCollection).to.deep.equal(sampleFeatureCollection);
			expect(raw).to.deep.equal(sampleResponse);
			done();
		});
	...
	it('should use a service to execute the request', (done) => {
		...
		const request = service.identify().on(map).at(latlng).run((error, featureCollection, raw) => {
			expect(featureCollection).to.deep.equal(sampleFeatureCollection);
			expect(raw).to.deep.equal(sampleResponse);
			done();
		});
	...
	it('should use a service to execute the request with simple LatLng', (done) => {
		...
		const request = service.identify().on(map).at(rawLatlng).run((error, featureCollection, raw) => {
			expect(featureCollection).to.deep.equal(sampleFeatureCollection);
			expect(raw).to.deep.equal(sampleResponse);
			done();
		});

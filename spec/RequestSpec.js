...
describe('L.esri request helpers', () => {
	...
	it('should be able to make a POST request with CORS', (done) => {
		L.esri.post('http://services.arcgisonline.com/ArcGIS/rest/info', {}, (error, response) => {
			expect(response).to.deep.equal(sampleResponse);
			done();
		});
	...
	it('should make a request with a token', (done) => {
		L.esri.request('http://services.arcgisonline.com/ArcGIS/rest/info', {token: 'foo'}, (error, response) => {
			expect(response).to.deep.equal(sampleResponse);
			done();
		});
	...
	it('should throw errors when response is not a JSON object', (done) => {
		L.esri.get.CORS('http://services.arcgisonline.com/ArcGIS/rest/info', {}, (error) => {
			expect(error).to.deep.equal({
	...
	it('should callback with an error when an XMLHttpRequest error is encountered', (done) => {
		const request = L.esri.post('http://services.arcgisonline.com/ArcGIS/rest/info', {}, (error, response) => {
			expect(error).to.deep.equal({
				error: {
	...
	it('should encode params for a request with params with apostrophes', (done) => {
		...
		L.esri.request('http://services.arcgisonline.com/ArcGIS/rest/info',
			{id: quotedString}, (error, response) => {
				expect(response).to.deep.equal(sampleResponse);
				done();
			});
